...
source config/ports.conf 2>/dev/null || {
...
start_ai_terminal() {
    ...
    if [ ! -f "src/app.py" ]; then
        echo -e "${RED}❌ Plik src/app.py nie został znaleziony${NC}"
        return 1
    fi
    
    python3 src/app.py >logs/app.log 2>&1 &
    sleep 3
...
show_logs() {
    ...
    if [ -f "logs/app.log" ]; then
        ...
        echo "Brak pliku logów logs/app.log"
    fi
}
...
case "${1:-help}" in
    ...
    clean)
        ...
        ;;
    version)
        echo -e "${BLUE}TTKi-cli v$VERSION${NC}"
        echo "Terminal AI + Desktop Environment"
        echo ""
        ;;
    help|--help|-h)
